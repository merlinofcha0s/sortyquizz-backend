...
/**
 * REST controller for managing {@link fr.sortyquizz.domain.Card}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CardResource {
     ...
     */
    @GetMapping("/cards")
    public ResponseEntity<List<Card>> getAllCards(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Cards");
        Page<Card> page;
        if (eagerload) {
            page = cardRepository.findAllWithEagerRelationships(pageable);
        } else {
    ...
    public ResponseEntity<Card> getCard(@PathVariable Long id) {
        ...
        Optional<Card> card = cardRepository.findOneWithEagerRelationships(id);
